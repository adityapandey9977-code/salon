...
export function requireGatewayPermission(...requiredPermissions) {
    return (req, _res, next) => {
        ...
        if (!hasPermission) {
            throw new ForbiddenError(`Forbidden: Missing permission ${requiredPermissions.join(' or ')}`);
        }
        next();
    };
}
export function enforceTenantScope(req, _res, next) {
    if (!req.user) {
        throw new UnauthorizedError('Authentication required');
    }
    if (req.user.scopeType === 'PLATFORM' || req.user.userType === 'PLATFORM') {
    ...
    const requestedTenantId = req.params.tenantId || req.query.tenantId || req.headers['x-tenant-id'];
...
export function enforceBranchScope(req, _res, next) {
    ...
    if (req.user.scopeType === 'PLATFORM' ||
