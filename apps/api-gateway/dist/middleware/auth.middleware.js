...
export async function gatewayAuthMiddleware(req, res, next) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new UnauthorizedError('Failed to resolve authorization context from Identity Service');
        }
        ...
        if (context.tenantCredentialId)
            req.headers['x-tenant-credential-id'] = context.tenantCredentialId;
        if (context.userType)
            req.headers['x-user-type'] = context.userType;
        if (context.tenantId)
            req.headers['x-tenant-id'] = context.tenantId;
        if (context.franchiseId)
            req.headers['x-franchise-id'] = context.franchiseId;
        ...
        logger.error({ error: err.message, sub, principalType }, 'Error communicating with Identity Service for auth context');
