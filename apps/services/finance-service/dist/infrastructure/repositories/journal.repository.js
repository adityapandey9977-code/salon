...
export class JournalRepository {
    ...
    async createAndPostJournal(tenantId, data) {
        ...
        if (Math.abs(totalDebit - totalCredit) > 0.01) {
            throw new BadRequestError(`Journal is unbalanced: Total Debits (${totalDebit}) do not equal Total Credits (${totalCredit})`);
        }
    ...
    async createReversalJournal(tenantId, originalJournalId, reversalNumber, reason) {
        const original = await this.findById(tenantId, originalJournalId);
        if (!original)
            throw new BadRequestError(`Original journal ${originalJournalId} not found`);
