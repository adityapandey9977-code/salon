...
export class RoyaltyService {
  constructor(
    private royaltyRepo: RoyaltyRepository = new RoyaltyRepository(),
    private cache: FinanceReadStore = new FinanceReadStore()
  ) {}

  async listRules(tenantId: string, franchiseId?: string) {
  ...
  async setBranchOverride(data: any) {
  ...
  async calculateAndRecordRoyalty(data: {
    ...
    let calcType: CalculationType = CalculationType.PERCENTAGE;
    let percentage: number | null = null;
    let fixedAmount: number | null = null;
    let basis: RoyaltyRevenueBasis = RoyaltyRevenueBasis.GROSS_SALES;
    ...
    if (basis === RoyaltyRevenueBasis.NET_SALES) eligibleRevenue = data.netSales;
    if (basis === RoyaltyRevenueBasis.SERVICE_REVENUE) eligibleRevenue = data.serviceRevenue;
  ...
  async generateSettlement(tenantId: string, data: {
    franchiseId: string;
    periodStart: Date;
    periodEnd: Date;
