...
export class TransferRepository {
  async listTransfers(tenantId: string, filter?: { status?: TransferStatus; sourceBranchId?: string; destinationBranchId?: string }) {
    ...
    if (filter?.sourceBranchId) where.sourceBranchId = filter.sourceBranchId;
    if (filter?.destinationBranchId) where.destinationBranchId = filter.destinationBranchId;
  ...
  async dispatchTransfer(id: string, dispatchedQuantities?: Record<string, number>) {
  ...
  async receiveTransfer(id: string, receivedQuantities?: Record<string, number>) {
    return prisma.$transaction(async (tx) => {
      const transfer = await tx.stockTransfer.findUniqueOrThrow({
        where: { id },
        include: { items: true },
