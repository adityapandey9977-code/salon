...
export class StocktakeService {
    ...
    skuRepo;
    cache;
    constructor(stocktakeRepo = new StocktakeRepository(), stockRepo = new StockRepository(), movementRepo = new MovementRepository(), skuRepo = new SkuRepository(), cache = new InventoryReadStore()) {
    ...
    async getStocktakeById(tenantId, id) {
        const stocktake = await this.stocktakeRepo.findById(tenantId, id);
        if (!stocktake)
            throw new NotFoundError(`Stocktake ${id} not found`);
        return stocktake;
    }
    ...
    async completeAndAdjust(tenantId, id, actorUserId) {
        ...
        for (const item of stocktake.items) {
            ...
            if (variance !== 0) {
                ...
                await this.stockRepo.updateStockProjection(tenantId, stocktake.branchId, item.skuId, variance);
