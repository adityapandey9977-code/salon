...
export class CustomerService {
    ...
    async getCustomerDetail(tenantId, customerId) {
        // 1. Redis Cache Read
    ...
    async lookupByMobile(tenantId, mobile) {
    ...
    async createCustomer(tenantId, input, userId = null, correlationId) {
        ...
        const created = await customerRepository.create({
            ...
            segment: input.segment || 'New Client',
            address: input.address?.addressLine1 && input.address?.city
                ? {
    ...
    async updateCustomer(tenantId, customerId, input, userId = null, correlationId) {
        const updateData = {};
        ...
        if (input.preferredBranchId !== undefined)
            updateData.preferredBranchId = input.preferredBranchId;
        if (input.franchiseId !== undefined)
            updateData.franchiseId = input.franchiseId;
    ...
    async softDeleteCustomer(tenantId, customerId, userId = null, reason, correlationId) {
    ...
    async recordVisit(tenantId, customerId, amount, visitDate) {
