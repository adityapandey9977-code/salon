...
export class BookingService {
    ...
    async create(tenantId, data, actor) {
        ...
        for (const item of data.items) {
            const serviceCtx = await externalServicesClient.getServiceBookingContext(tenantId, item.serviceId, data.branchId);
            ...
            if (item.staffId) {
                const staffAvailability = await externalServicesClient.getStaffAvailability(tenantId, item.staffId, data.branchId, itemStart.toISOString().split('T')[0]);
        ...
        const lockAcquired = await SlotLockManager.acquireLock(tenantId, data.branchId, primaryStaff, startAt.toISOString(), lockToken);
        if (!lockAcquired) {
        ...
        try {
            ...
            for (const item of processedItems) {
                if (item.staffId) {
                    const hasConflict = await appointmentRepository.checkConflicts(tenantId, data.branchId, item.staffId, item.scheduledStartAt, item.scheduledEndAt);
                    if (hasConflict) {
            ...
            // 6. Invalidate Cache
            await bookingReadStore.invalidateAppointment(tenantId, record.id, data.branchId, startAt.toISOString().split('T')[0]);
            // 7. Publish Event
            ...
            await SlotLockManager.releaseLock(tenantId, data.branchId, primaryStaff, startAt.toISOString(), lockToken);
        }
    }
    async updateStatus(tenantId, id, status, actor, reason) {
        const updated = await appointmentRepository.updateStatus(tenantId, id, status, actor, reason);
        await bookingReadStore.invalidateAppointment(tenantId, updated.id, updated.branchId, updated.scheduledStartAt.split('T')[0]);
        ...
        if (status === 'COMPLETED') {
            const visitAmount = updated.subtotalEstimate ||
                (updated.items || []).reduce((sum, it) => sum + (it.priceSnapshot || 0), 0);
            externalServicesClient
                .recordCustomerVisit(tenantId, updated.customerId, visitAmount, updated.completedAt || new Date().toISOString())
                .catch(() => { });
        }
        return updated;
    }
    async reassignStaff(tenantId, appointmentId, itemId, newStaffId) {
        const record = await appointmentRepository.reassignStaff(tenantId, appointmentId, itemId, newStaffId);
        await bookingReadStore.invalidateAppointment(tenantId, appointmentId, record.branchId);
        return record;
    }
    async getCalendar(tenantId, branchId, date) {
    ...
    async getTodayQueue(tenantId, branchId) {
    ...
    async getWalkins(tenantId, branchId) {
        const todayStr = new Date().toISOString().split('T')[0];
        const startDate = new Date(`${todayStr}T00:00:00.000Z`);
        const endDate = new Date(`${todayStr}T23:59:59.999Z`);
        const result = await appointmentRepository.list(tenantId, {
    ...
    async createWalkin(tenantId, data, actor) {
        const now = new Date();
    ...
    async getMyBookings(tenantId, customerId) {
        const result = await appointmentRepository.list(tenantId, {
            customerId,
            limit: 50,
        });
    ...
    async getPendingConfirmations(tenantId, branchId) {
        const result = await appointmentRepository.list(tenantId, {
            branchId,
    ...
    async getCrossBranch(tenantId, customerId) {
        const result = await appointmentRepository.list(tenantId, {
            customerId,
            limit: 50,
        });
