...
export class AvailabilityService {
    ...
    async getAvailableSlots(params) {
        ...
        let bufferAfter = 5;
        if (serviceId) {
            const serviceContext = await externalServicesClient.getServiceBookingContext(tenantId, serviceId, branchId);
            if (serviceContext) {
        ...
        for (let hour = openHour; hour < closeHour; hour++) {
            for (const minute of [0, 30]) {
                ...
                const hasConflict = await appointmentRepository.checkConflicts(tenantId, branchId, staffId, slotStart, slotEnd);
                // Check active booking holds
                const hasHold = await holdRepository.checkActiveHolds(tenantId, branchId, slotStart, slotEnd);
                slots.push({
