...
export class AppointmentRepository {
    ...
    async checkConflicts(tenantId, branchId, staffId, startTime, endTime, excludeAppointmentId, tx) {
        if (!staffId)
            return false;
        ...
        const record = await prisma.$transaction(async (tx) => {
            ...
            for (const item of data.items) {
                if (item.staffId) {
                    ...
                    const hasConflict = await this.checkConflicts(data.tenantId, data.branchId, item.staffId, item.scheduledStartAt, item.scheduledEndAt, undefined, tx);
                    if (hasConflict) {
    ...
    async updateStatus(tenantId, id, newStatus, actor, reason) {
        const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id);
    ...
    async reassignStaff(tenantId, appointmentId, itemId, newStaffId) {
        ...
        if (!targetItem)
            throw new NotFoundError('Appointment item not found');
        // Check conflict for new staff member
        const hasConflict = await this.checkConflicts(tenantId, targetAppointment.branchId, newStaffId, targetItem.scheduledStartAt, targetItem.scheduledEndAt, targetAppointment.id);
        if (hasConflict) {
