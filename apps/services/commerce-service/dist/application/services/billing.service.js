...
export class BillingService {
    ...
    async calculateTax(tenantId, input) {
    ...
    async checkout(tenantId, input, userId = null, correlationId) {
        // 1. Calculate Tax & Line Items
        const taxResult = calculateCartTax(input.items.map((it) => ({
            itemType: it.itemType,
            unitPrice: it.unitPrice,
            quantity: it.quantity,
            discountAmount: it.discountAmount,
    ...
    async handlePaymentCompleted(tenantId, invoiceId, amount, userId = null, correlationId) {
    ...
    async handlePaidInvoice(tenantId, invoice, userId = null, correlationId) {
    ...
    async requestRefund(tenantId, input, userId = null, correlationId) {
