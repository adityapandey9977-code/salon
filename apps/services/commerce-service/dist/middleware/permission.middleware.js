...
export function requirePermission(requiredPermission) {
    return (req, _res, next) => {
        ...
        const rolesUpper = (req.auth.roles || []).map((r) => String(r).toUpperCase().replace(/[\s-]+/g, '_'));
        const roleUpper = String(req.auth.role || '').toUpperCase().replace(/[\s-]+/g, '_');
        const hasBranchScope = Boolean(req.auth.branchIds && req.auth.branchIds.length > 0);
        const isBranchManager = rolesUpper.includes('BRANCH_MANAGER') ||
            roleUpper === 'BRANCH_MANAGER' ||
            hasBranchScope;
        const isReadOperation = requiredPermission.endsWith('.read') ||
        ...
        const permissions = (req.auth.permissions || []).map((p) => p.toLowerCase().trim());
