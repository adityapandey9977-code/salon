...
import { Prisma, type MembershipMaster } from '../prisma/generated-client';

export class MembershipRepository {
  private toDto(item: MembershipMaster): MembershipMasterDto {
    return {
      id: item.id,
  ...
  public async findById(tenantId: string, id: string): Promise<MembershipMasterDto | null> {
  ...
  public async list(tenantId: string): Promise<MembershipMasterDto[]> {
  ...
  public async update(
    ...
    const updateData: Prisma.MembershipMasterUpdateInput = {};
    ...
    if (data.benefitsJson !== undefined) updateData.benefitsJson = data.benefitsJson;
    ...
    if (data.perksText !== undefined) updateData.perksText = data.perksText;
  ...
  public async createCustomerMembership(data: {
    tenantId: string;
    customerId: string;
  ...
  public async listMembershipBenefits(tenantId: string) {
    ...
    plans.forEach((plan) => {
      if (plan.benefitsJson && typeof plan.benefitsJson === 'object') {
        const obj = plan.benefitsJson as Record<string, any>;
        if (Array.isArray(obj.perks)) {
          obj.perks.forEach((p: any, idx: number) => {
            benefits.push({
  ...
  public async createMembershipBenefit(data: {
    tenantId: string;
    membershipPlanId?: string;
    perkName: string;
    category?: string;
    ...
    if (data.membershipPlanId) {
      ...
      if (plan) {
        const existingJson = (plan.benefitsJson as Record<string, any>) || {};
  ...
  public async renewMembership(data: {
    tenantId: string;
