...
type ServiceWithRelations = ServiceMaster & {
  branchPrices?: ServiceBranchPrice[];
};

export class ServiceRepository {
  private toDto(item: ServiceWithRelations): CachedServiceMaster {
    return {
      id: item.id,
      ...
      availableBranches:
        (item as any).availableBranches && (item as any).availableBranches.length > 0
          ? (item as any).availableBranches
          : (item.metadata as any)?.availableBranches || [],
  ...
  private toCategoryDto(item: ServiceCategory): ServiceCategoryDto {
    return {
      id: item.id,
      tenantId: item.tenantId,
      name: item.name,
  ...
  public async deleteCategory(tenantId: string, id: string): Promise<void> {
    ...
    if (activeServicesCount > 0) {
      throw new ConflictError(
        `Cannot delete category with ${activeServicesCount} linked service(s). Please reassign or delete them first.`,
      );
    }
  ...
  // Services
  public async findById(tenantId: string, id: string): Promise<CachedServiceMaster | null> {
  ...
  public async list(
    ...
    if (filters.isBookableOnline !== undefined) where.isBookableOnline = filters.isBookableOnline;
  ...
  public async create(data: {
    ...
    try {
      const branches = Array.isArray(data.availableBranches)
        ...
        : Array.isArray((data.metadata as any)?.availableBranches)
          ? (data.metadata as any).availableBranches
          : [];
  ...
  public async setBranchPrice(data: {
    tenantId: string;
    serviceId: string;
    branchId: string;
    price: number;
  ...
  public async setRecipe(data: {
    tenantId: string;
    serviceId: string;
    name?: string;
    description?: string | null;
    version?: number;
    items: Array<{ skuId: string; quantityRequired: number; unit?: string }>;
  }): Promise<void> {
    const service = await prisma.serviceMaster.findFirst({
      where: { id: data.serviceId, tenantId: data.tenantId, deletedAt: null },
    });
  ...
  private isSkillsTableReady = false;

  public async ensureSkillsTable(): Promise<void> {
    if (this.isSkillsTableReady) return;
    try {
  ...
  public async listSkills(tenantId: string): Promise<any[]> {
    ...
    try {
      const distinctServiceSkills: any[] = await prisma.$queryRawUnsafe(
        `SELECT DISTINCT sm.required_skill, sm.category_id, sc.name as category_name
      ...
      for (const ds of distinctServiceSkills) {
        ...
        const exists: any[] = await prisma.$queryRawUnsafe(
          `SELECT id FROM catalogue_skills WHERE tenant_id = $1::uuid AND LOWER(name) = LOWER($2) LIMIT 1`,
          tenantId,
          skillName,
        );
        if (exists.length === 0) {
          ...
          await prisma.$executeRawUnsafe(
            `INSERT INTO catalogue_skills (tenant_id, code, name, category_id, category_name, description, status)
            ...
            skillName,
            ds.category_id || null,
            ds.category_name || 'General Services',
            `Master technical competency required for ${skillName} services.`,
          );
        }
      }
    } catch (syncErr) {
    ...
    const rawSkills: any[] = await prisma.$queryRawUnsafe(
      `SELECT id, tenant_id, code, name, category_id, category_name, description, status, created_at, updated_at
    ...
    try {
      const serviceCounts: any[] = await prisma.$queryRawUnsafe(
        `SELECT required_skill, COUNT(*)::int as count
    ...
    return rawSkills.map((r) => {
      const linked =
        countMap[r.name.toLowerCase()] ?? countMap[r.code.toLowerCase()] ?? 0;
      return {
        id: r.id,
  ...
  public async seedStandardSkills(tenantId: string): Promise<any[]> {
    await this.ensureSkillsTable();

    const categories = await this.listCategories(tenantId);
    const hairCat =
      categories.find((c) => c.name.toLowerCase().includes('hair'))?.name ||
    ...
    const nailCat =
      categories.find((c) => c.name.toLowerCase().includes('nail'))?.name ||
      'Nails Art & Spa Lounge';
    const makeupCat =
      categories.find(
        (c) =>
          c.name.toLowerCase().includes('bridal') ||
    ...
    const defaultSeeds = [
      {
        ...
        description:
          'Structural sectioning, bespoke layered cuts, and razor feathering techniques.',
      },
      {
        code: 'SKL-CHEM-02',
        ...
        categoryName: hairCat,
        description:
          'Keratin, cysteine, balayage bleaching, and ammonia-free root melt chemistry.',
      },
      {
        code: 'SKL-SKIN-03',
        ...
        description:
          'Hydrodermabrasion, high-frequency ozone therapy, chemical peel neutralisation.',
      },
      {
        code: 'SKL-SPA-04',
        ...
        categoryName: spaCat,
        description:
          'Swedish effleurage, lymphatic drainage, reflexology point pressure, hot stones.',
      },
      {
        code: 'SKL-NAIL-05',
        ...
        description:
          'Polygel apex sculpting, electric file cuticle prep, chrome encapsulation.',
      },
      {
        code: 'SKL-MAKE-06',
        ...
        description:
          'High-definition micro-pigment misting, contouring, silicone bridal sealing.',
      },
    ];
  ...
  public async createSkill(
    ...
    const existing: any[] = await prisma.$queryRawUnsafe(
      `SELECT id FROM catalogue_skills WHERE tenant_id = $1::uuid AND UPPER(code) = $2 LIMIT 1`,
      tenantId,
      cleanCode,
    );
    ...
    const inserted: any[] = await prisma.$queryRawUnsafe(
      `INSERT INTO catalogue_skills (tenant_id, code, name, category_id, category_name, description, status)
       ...
       RETURNING id, tenant_id, code, name, category_id, category_name, description, status, created_at, updated_at`,
      tenantId,
      cleanCode,
      data.name.trim(),
      categoryIdVal,
  ...
  public async updateSkill(
    ...
    if (data.code) {
      const cleanCode = data.code.trim().toUpperCase();
      const existing: any[] = await prisma.$queryRawUnsafe(
        `SELECT id FROM catalogue_skills WHERE tenant_id = $1::uuid AND UPPER(code) = $2 AND id != $3::uuid LIMIT 1`,
        tenantId,
        cleanCode,
        id,
      );
    ...
    const currentRaw: any[] = await prisma.$queryRawUnsafe(
      `SELECT * FROM catalogue_skills WHERE tenant_id = $1::uuid AND id = $2::uuid LIMIT 1`,
      tenantId,
      id,
    );
  ...
  public async deleteSkill(tenantId: string, id: string): Promise<void> {
    await this.ensureSkillsTable();
    await prisma.$executeRawUnsafe(
      `DELETE FROM catalogue_skills WHERE tenant_id = $1::uuid AND id = $2::uuid`,
      tenantId,
      id,
    );
  }
