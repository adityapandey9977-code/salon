...
export class NotificationService {
  constructor(
    private notifRepo: NotificationRepository = new NotificationRepository(),
    private templateService: TemplateService = new TemplateService()
  ) { }

  async listNotifications(tenantId: string, filter?: any) {
    return this.notifRepo.listNotifications(tenantId, filter);
  }

  async listAllLogs(filter?: any) {
    return this.notifRepo.listAllLogs(filter);
  }

  async resolveNotification(tenantId: string, id: string) {
    const notif = await this.notifRepo.findById(tenantId, id);
    if (!notif) throw new NotFoundError(`Notification ${id} not found`);
  ...
  async sendNotification(data: {
    ...
    recipientId: string; // Phone number or email address or user UUID
    channel: Channel;
    templateCode?: string;
    subject?: string;
    body?: string;
