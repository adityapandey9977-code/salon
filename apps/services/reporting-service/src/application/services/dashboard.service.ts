...
export class DashboardService {
  public static async getDashboardMetrics(tenantId: string, branchId?: string, dateRange = '30d') {
  ...
  public static async getDashboardCharts(tenantId: string, branchId?: string, period = 'month') {
  ...
  public static async getOccupancy(tenantId: string, branchId?: string, _date?: string) {
  ...
  public static async getRecentActivity(tenantId: string, _branchId?: string, _limit = 10) {
  ...
  public static async getBranchKpis(tenantId: string, branchId: string) {
  ...
  public static async getCallCenterKpis(tenantId: string, dateRange = '30d') {
    const startDate = this.calculateStartDate(dateRange);
  ...
  public static async getBranchComparison(tenantId: string, dateRange = '30d') {
    const startDate = this.calculateStartDate(dateRange);
    const branchMetrics = await MetricsRepository.getBranchMetrics(tenantId, undefined, startDate);

    const grouped: Record<string, { branchId: string; totalRevenue: number; totalAppointments: number }> = {};
  ...
  public static async getSuperAdminKpis() {
  ...
  public static async getSuperAdminMrrTelemetry() {
    return {
      currentMrr: 125000.0,
      currency: 'USD',
  ...
  public static async getSuperAdminTenantHealth() {
    return {
      healthyTenants: 48,
      atRiskTenants: 2,
      suspendedTenants: 0,
  ...
  public static async getSuperAdminSystemMetrics() {
    return {
  ...
  public static async getInventoryDashboardKpis(tenantId: string, branchId?: string) {
  ...
  public static async getFranchiseDashboardKpis(tenantId: string, franchiseId: string) {
  ...
  public static async getFranchiseSalesTrend(tenantId: string, franchiseId: string) {
    const metrics = await MetricsRepository.getFranchiseMetrics(tenantId, franchiseId);
    return {
      franchiseId,
      trend: metrics.map((m) => ({
  ...
  public static async getFranchiseSalesSummary(tenantId: string, franchiseId: string) {
    const metrics = await MetricsRepository.getFranchiseMetrics(tenantId, franchiseId);
  ...
  public static async getFranchiseStaffSummary(tenantId: string, _franchiseId: string) {
  ...
  public static async getFranchiseInventorySummary(tenantId: string, _franchiseId: string) {
  ...
  private static calculateStartDate(range: string): Date {
    const now = new Date();
