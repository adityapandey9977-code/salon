...
export class AttendanceService {
  public async clockIn(
    ...
    if (employee.employmentStatus !== 'ACTIVE') {
      throw new ConflictError(
        `Employee cannot clock in because status is ${employee.employmentStatus}`,
      );
    }

    // Verify employee is assigned to the specified branch
    const branchAssignment = await staffBranchRepository.findAssignment(
      tenantId,
      employeeId,
      data.branchId,
    );
    ...
    const existingForToday = await attendanceRepository.findByEmployeeAndDate(
      tenantId,
      employeeId,
      today,
    );
  ...
  public async getAttendanceForEmployee(
    tenantId: string,
    employeeId: string,
    dateStr?: string,
  ): Promise<AttendanceRecordDto | AttendanceRecordDto[]> {
    if (dateStr) {
