...
export class AvailabilityService {
  public async getStaffAvailabilityContext(
    tenantId: string,
    employeeId: string,
    queryDate?: string,
  ): Promise<StaffAvailabilityContextResponse> {
  ...
  public async getBookableStaffForBranch(
    tenantId: string,
    branchId: string,
    _date?: string,
    serviceId?: string,
    ...
    for (const id of employeeIds) {
      ...
      if (emp && emp.employmentStatus === 'ACTIVE') {
        ...
        const isBookable =
          (emp.profile?.isBookable ?? true) && (emp.profile?.acceptsOnlineBooking ?? true);
