...
export class EmployeeRepository {
    toSafeProfile(emp) {
        ...
        const employmentType = (empTypeMap[emp.employmentType] || 'Full Time');
    ...
    async findByIdentityUserId(tenantId, identityUserId) {
        const emp = await prisma.employee.findFirst({
            where: {
                tenantId,
                identityUserId,
    ...
    async findByIdentityUserIdGlobal(identityUserId) {
        const emp = await prisma.employee.findFirst({
            where: {
                identityUserId,
                deletedAt: null,
    ...
    async generateNextEmployeeCode(tenantId) {
    ...
    async listEmployees(params) {
        ...
        if (params.sortBy === 'displayName')
            orderBy.displayName = params.sortOrder || 'asc';
        else if (params.sortBy === 'employeeCode')
            orderBy.employeeCode = params.sortOrder || 'asc';
        else if (params.sortBy === 'joiningDate')
            orderBy.joiningDate = params.sortOrder || 'desc';
        else
    ...
    async createEmployee(data) {
        const created = await prisma.$transaction(async (tx) => {
            ...
            const specialization = data.profile?.specialization ||
                (Array.isArray(data.skills) && data.skills.length > 0
                    ? data.skills.join(', ')
                    : null) ||
                (Array.isArray(data.assignedServices) && data.assignedServices.length > 0
                    ? data.assignedServices.join(', ')
                    : null) ||
                null;
            const profile = await tx.staffProfile.create({
                data: {
                    ...
                    yearsOfExperience: data.profile?.yearsOfExperience !== undefined && data.profile?.yearsOfExperience !== null
    ...
    async updateEmployee(tenantId, id, data, changedByIdentityUserId) {
        const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id);
        const updated = await prisma.$transaction(async (tx) => {
            ...
            const specialization = data.profile?.specialization !== undefined
    ...
    async softDeleteEmployee(tenantId, id, deletedByIdentityUserId, deleteReason) {
        const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id);
