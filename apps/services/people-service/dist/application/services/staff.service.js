...
export class StaffService {
    async listStaff(tenantId, query) {
        const hasFranchise = query.hasFranchise === true ||
    ...
    async getStaffDetail(tenantId, id) {
    ...
    async getStaffMe(identityUserId, tenantId) {
    ...
    async createStaff(tenantId, data, createdByUserId) {
        ...
        const isBranchManagerRole = String(rawRole).toUpperCase().includes('BRANCH_MANAGER') ||
        ...
        const shouldEnableLogin = anyData.loginEnabled ??
        ...
        if (shouldEnableLogin && employee.email) {
            try {
                ...
                if (identityRes.ok) {
                    ...
                    console.warn('[StaffService] Identity Service response not ok:', await identityRes.text());
                }
            }
            catch (identityErr) {
                console.warn('[StaffService] Could not reach Identity Service to provision staff login:', identityErr);
            }
        }
    ...
    async updateStaff(tenantId, id, data, updatedByUserId) {
        const existing = await employeeRepository.findById(tenantId, id);
        if (!existing) {
            throw new NotFoundError('Employee not found');
        }
        const updated = await employeeRepository.updateEmployee(tenantId, id, {
            firstName: data.firstName,
            lastName: data.lastName,
            displayName: data.displayName,
            email: data.email,
            ...
            primaryBranchId: data.primaryBranchId && isUuid(data.primaryBranchId)
                ? data.primaryBranchId
                : data.primaryBranchId === null
                    ? null
                    : undefined,
            franchiseId: data.franchiseId && isUuid(data.franchiseId)
                ? data.franchiseId
            ...
            managerEmployeeId: data.managerEmployeeId && isUuid(data.managerEmployeeId)
            ...
            notes: data.notes,
            profile: data.profile || data.skills
                ? {
                    ...data.profile,
                    specialization: data.profile?.specialization !== undefined
        ...
        const isBranchManagerRole = String(rawRole).toUpperCase().includes('BRANCH_MANAGER') ||
            ...
            String(anyData.role || '').toLowerCase().includes('branch manager');
        const normalizedRoleCode = isBranchManagerRole ? 'BRANCH_MANAGER' : (anyData.roleCode || anyData.role || 'STYLIST');
        const shouldUpdateLogin = anyData.loginEnabled ??
    ...
    async softDeleteStaff(tenantId, id, deletedByUserId, deleteReason) {
        const existing = await employeeRepository.findById(tenantId, id);
        if (!existing) {
            throw new NotFoundError('Employee not found');
        }
        const deleted = await employeeRepository.softDeleteEmployee(tenantId, id, deletedByUserId, deleteReason);
        await peopleReadStore.invalidateEmployee(tenantId, id, existing.primaryBranchId);
        await eventPublisher.publishEvent({
    ...
    async getBranchTeam(tenantId, branchId) {
