...
router.get('/:id/branches', requirePermission('staff.read', 'staff.branch.manage'), (req, res, next) => {
...
router.patch('/:id/branches/:assignmentId', requirePermission('staff.branch.manage'), (req, res, next) => {
    branchAssignmentController.updateAssignment(req, res).catch(next);
});
router.delete('/:id/branches/:assignmentId', requirePermission('staff.branch.manage'), (req, res, next) => {
    branchAssignmentController.removeAssignment(req, res).catch(next);
});
// Skills Sub-resource
router.get('/:id/skills', requirePermission('staff.skill.read', 'staff.skill.manage', 'staff.read'), (req, res, next) => {
...
router.get('/:id/leave-balances', requirePermission('leave.read', 'leave.manage', 'staff.read'), (req, res, next) => {
    leaveController.getLeaveBalances(req, res).catch(next);
});
router.post('/:id/leave-balances/adjust', requirePermission('leave.manage'), (req, res, next) => {
