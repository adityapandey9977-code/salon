...
export class ProvisioningService {
    ...
    domainRepo;
    cache;
    constructor(provRepo = new ProvisioningRepository(), planRepo = new PlanRepository(), subRepo = new SubscriptionRepository(), domainRepo = new DomainRepository(), cache = new PlatformReadStore()) {
    ...
    async getRequestById(id) {
        ...
        if (!req)
            throw new NotFoundError(`Provisioning request ${id} not found`);
        return req;
    }
    async provisionTenant(data) {
        ...
        try {
            ...
            const orgRes = await fetch(`${config.ORGANIZATION_SERVICE_URL}/api/v1/tenants`, {
                method: 'POST',
                headers: {
            ...
            const identRes = await fetch(`${config.IDENTITY_SERVICE_URL}/internal/v1/tenant-credentials`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'x-internal-token': config.SERVICE_INTERNAL_SECRET,
