...
// ---------------------------------------------------------------------------
// In-memory store for support tickets (persisted across requests in one
// process; suitable until a DB migration adds a proper tickets table).
// ---------------------------------------------------------------------------
...
const ticketStore: SupportTicket[] = [
  {
    id: 'TCK-FRN-901',
...
export class SupportController {
  ...
  static listTickets(req: Request, res: Response, next: NextFunction) {
    try {
      const { tenantId, status, priority, category } = req.query as Record<string, string>;

      let results = [...ticketStore];

      // Filter by tenantId extracted from query or JWT header x-tenant-id
      const resolvedTenantId = tenantId || req.headers['x-tenant-id'] as string;
      if (resolvedTenantId) {
  ...
  static getTicket(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  static createTicket(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const ticket: SupportTicket = {
        id: generateTicketId(),
        tenantId: tenantId || (req.headers['x-tenant-id'] as string) || 'unknown',
  ...
  static updateTicketStatus(req: Request, res: Response, next: NextFunction) {
    try {
