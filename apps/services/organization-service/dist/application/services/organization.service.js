...
async function resolvePublicServerIp(req) {
    const now = Date.now();
...
export class OrganizationService {
    async getDnsInfo(req) {
        ...
        const cleanHost = hostHeader.split(':')[0];
        const cnameTarget = cleanHost.includes('localhost') || cleanHost.includes('127.0.0.1')
    ...
    async createTenant(input) {
        ...
        try {
            const res = await fetch(`${config.IDENTITY_SERVICE_URL}/internal/v1/tenant-credentials`, {
                method: 'POST',
                headers: {
            ...
            if (!res.ok) {
                ...
                console.warn(`[OrganizationService] Identity Service credential response ${res.status}: ${errText}`);
            }
        }
        catch (identityErr) {
            console.warn('[OrganizationService] Could not connect to Identity Service for credential provisioning:', identityErr);
        }
    ...
    async getBranches(tenantId, franchiseId) {
        ...
        return branches.map((b) => ({
            ...
            name: b.name,
            code: b.code,
            type: (b.franchiseId ? 'Franchise' : 'Flagship'),
            ...
            phone: b.phone,
            email: b.email,
            status: (b.status === 'ACTIVE' ? 'Active' : b.status === 'TEMPORARILY_CLOSED' ? 'Pending' : 'Inactive'),
    ...
    async updateBranch(branchId, input) {
        ...
        if (resolvedFranchiseId && resolvedFranchiseId !== existing.franchiseId) {
            ...
            if (franchise) {
                ...
                if (currentCount >= allowedLimit) {
                    throw new BadRequestError(`Franchise partner "${franchise.companyName}" has reached its authorized outlet limit of ${allowedLimit} outlet(s) (currently has ${currentCount}). Cannot assign branch.`);
                }
            }
        }
        const updated = await prisma.branch.update({
            ...
            data: {
                ...
                primaryManagerEmployeeId: input.primaryManagerEmployeeId !== undefined
        ...
        return {
            ...updated,
            type: (updated.franchiseId ? 'Franchise' : 'Flagship'),
    ...
    async updateOperatingHours(branchId, schedules) {
    ...
    async getHolidays(tenantId, branchId) {
    ...
    async createHoliday(branchId, input) {
    ...
    async getFranchisePartners(tenantId) {
        if (tenantId) {
    ...
    async getFranchisePartnerById(partnerId) {
    ...
    async updateFranchisePartner(partnerId, input) {
        ...
        if (input.companyName || input.name)
            dataToUpdate.companyName = (input.companyName || input.name).trim();
        if (input.code !== undefined)
            dataToUpdate.code = input.code ? String(input.code).trim() : null;
        if (input.contactPerson || input.contact)
            dataToUpdate.contactPerson = (input.contactPerson || input.contact).trim();
        if (input.contactPhone || input.phone)
            dataToUpdate.contactPhone = (input.contactPhone || input.phone).trim();
        if (input.address !== undefined)
            dataToUpdate.address = input.address ? String(input.address).trim() : null;
        ...
        if (input.status !== undefined)
            dataToUpdate.status = String(input.status).trim();
        ...
        if (input.gstin !== undefined)
            dataToUpdate.gstin = input.gstin ? String(input.gstin).trim() : null;
    ...
    async createFranchisePartner(tenantId, input) {
        ...
        const authorizedOutletsCount = typeof rawOutlets === 'number' ? rawOutlets : parseInt(String(rawOutlets || '1'), 10) || 1;
        const royaltyModel = (input.royaltyModel || input.royaltyStructure || '').trim() || null;
        const status = (input.status || 'ACTIVE').trim();
        let agreementStart = null;
    ...
    async createBranch(tenantId, input) {
        ...
        const rawFid = input.franchiseId || input.franchisePartnerId;
        if (rawFid && isUuid(rawFid)) {
            ...
            matchedFranchise = await prisma.franchisePartner.findFirst({
                where: {
                    companyName: {
                        contains: input.franchisePartnerName.trim(),
                        mode: 'insensitive',
                    },
                },
            });
        ...
        if (matchedFranchise) {
            ...
            if (franchiseWithCount) {
                ...
                if (currentCount >= allowedLimit) {
                    throw new BadRequestError(`Franchise partner "${franchiseWithCount.companyName}" has reached its authorized outlet limit of ${allowedLimit} outlet(s) (currently has ${currentCount}). Cannot create additional outlets.`);
                }
            }
        }
        ...
        let tenantExists = isUuid(resolvedTenantId)
            ...
            : null;
        if (!tenantExists) {
            const defaultTenant = (await prisma.organizationTenant.findFirst({ where: { status: 'ACTIVE' } })) ||
        ...
        return {
            ...branch,
            type: (branch.franchiseId ? 'Franchise' : 'Flagship'),
            franchisePartnerId: branch.franchiseId || undefined,
            franchisePartnerName: branch.franchise?.companyName || input.franchisePartnerName || undefined,
            isFranchiseOwned: Boolean(branch.franchiseId),
        };
    }
    async getResources(tenantId, branchId) {
    ...
    async updateResource(id, data) {
        const isAvail = data.isAvailable !== undefined
            ? data.isAvailable
     ...
     */
    async reprovisionCredentials(tenantId) {
        const tenant = await prisma.organizationTenant.findUnique({
            where: { id: tenantId },
        });
        if (!tenant) {
        ...
        if (!res.ok) {
            ...
            if (res.status === 409) {
                const patchRes = await fetch(`${config.IDENTITY_SERVICE_URL}/internal/v1/tenant-credentials/reset-password`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'x-service-secret': config.SERVICE_INTERNAL_SECRET,
    ...
    // ==========================================
    async getComplianceRequirements(tenantId) {
    ...
    async createComplianceRequirement(payload) {
    ...
    async deleteComplianceRequirement(idOrCode) {
        try {
