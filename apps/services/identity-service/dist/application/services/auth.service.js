...
export class AuthService {
    async login(params) {
        ...
        if (!isPasswordValid) {
            ...
            if (isLocked) {
                throw new AccountLockedError('Account has been locked due to consecutive failed login attempts.');
            }
            throw new AuthenticationError('Invalid email or password');
        }
        ...
        const primaryScope = user.userType === 'PLATFORM' ? 'PLATFORM' : effectiveAccess.tenantId ? 'TENANT' : 'PLATFORM';
    ...
    async loginSuperAdmin(params) {
        ...
        const resolvedPlatformRoles = effectiveAccess.roles.length > 0 ? effectiveAccess.roles : ['SUPER_ADMIN'];
        const primaryRole = resolvedPlatformRoles[0] || (user.userType === 'PLATFORM' ? 'SUPER_ADMIN' : 'USER');
        const accessToken = TokenService.createUserAccessToken({
            userId: user.id,
            sessionId: session.id,
            role: primaryRole,
    ...
    async refreshToken(rawRefreshToken) {
        ...
        const isTenant = storedToken.session.principalType === 'TENANT' ||
            Boolean(storedToken.session.tenantCredentialId);
        const isUser = storedToken.session.principalType === 'USER' ||
        ...
        const isWithinGracePeriod = storedToken.isUsed &&
            !storedToken.isRevoked &&
            storedToken.session.status === 'ACTIVE' &&
            new Date(storedToken.expiresAt) > new Date();
        if (isWithinGracePeriod) {
            logger.info({ sessionId: storedToken.sessionId }, 'Concurrent refresh request within grace window — issuing refreshed access token.');
            if (isTenant && storedToken.session.tenantCredentialId) {
                const credential = await tenantCredentialRepository.findById(storedToken.session.tenantCredentialId);
            ...
            if (isUser && storedToken.session.userId) {
                ...
                const primaryScope = user.userType === 'PLATFORM'
                    ? 'PLATFORM'
        ...
        if (storedToken.isRevoked || storedToken.isUsed) {
            ...
            if (isTenant && storedToken.session.tenantCredentialId) {
                await identityReadStore.invalidateAllTenantSessions(storedToken.session.tenantCredentialId);
            }
            if (isUser && storedToken.session.userId) {
        ...
        const { rawToken: newRefreshToken, tokenHash: newTokenHash, expiresAt: newExpiresAt, } = TokenService.generateRefreshToken(7);
        ...
        if (isUser && storedToken.session.userId) {
            ...
            const effectiveAccess = await this.getEffectiveAccess(user.id);
            const resolvedPlatformRoles = effectiveAccess.roles.length > 0
                ? effectiveAccess.roles
                : user.userType === 'PLATFORM'
                    ? ['SUPER_ADMIN']
                    : ['USER'];
            const primaryRole = resolvedPlatformRoles[0] ||
                (user.userType === 'PLATFORM' ? 'SUPER_ADMIN' : 'USER');
            const primaryScope = user.userType === 'PLATFORM'
                ? 'PLATFORM'
                : effectiveAccess.tenantId
