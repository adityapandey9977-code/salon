...
export class InternalController {
    async getAuthContext(req, res, next) {
        try {
            const principalType = req.query.principalType || req.headers['x-principal-type'];
            const userId = req.query.userId || req.headers['x-user-id'];
            const tenantCredentialId = req.query.tenantCredentialId ||
                req.headers['x-tenant-credential-id'];
            ...
            const primaryRole = effectiveAccess.roles[0] || (user.userType === 'PLATFORM' ? 'SUPER_ADMIN' : 'USER');
    ...
    async createTenantCredential(req, res, next) {
        try {
    ...
    async resetTenantCredentialPassword(req, res, next) {
        try {
    ...
    async createStaffUser(req, res, next) {
        try {
            const { tenantId, fullName, email, mobilePhone, password, roleId, roleCode, branchId, franchiseId, } = req.body;
    ...
    async assignStaffBranchScope(req, res, next) {
        try {
            const { userId, tenantId, branchId, roleCode } = req.body;
            if (!userId || !branchId) {
