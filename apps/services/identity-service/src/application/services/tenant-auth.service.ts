...
export class TenantAuthService {
   ...
   */
  private async validateOrganizationTenantStatus(tenantId: string): Promise<{
    isValid: boolean;
    status?: string;
    salonName?: string;
    tenantCode?: string;
    ...
    try {
      const orgServiceUrl =
        process.env.ORGANIZATION_SERVICE_URL || 'http://localhost:5002';
      const response = await fetch(
        `${orgServiceUrl}/internal/v1/tenants/${tenantId}`,
        {
          headers: {
      ...
      logger.warn(
        ...
        'Could not reach Organization Service to verify tenant status, continuing with credential status',
      );
      return { isValid: true, status: 'ACTIVE' };
    }
  }
  ...
  public async login(params: {
    ...
    const credential =
      await tenantCredentialRepository.findByNormalizedEmail(normalizedEmail);
    ...
    if (credential.lockedUntil && new Date(credential.lockedUntil) > new Date()) {
      ...
      throw new AccountLockedError(
        'Tenant account is temporarily locked due to consecutive failed attempts.',
      );
    }
    ...
    if (
      ...
      throw new UnauthorizedError(
        'Tenant account is suspended or disabled. Contact platform support.',
      );
    }
    ...
    if (!isPasswordValid) {
      const { isLocked } =
        await tenantCredentialRepository.handleFailedLogin(credential);
      await authRepository.recordLoginAttempt({
        email: normalizedEmail,
        ipAddress: params.ipAddress,
        userAgent: params.userAgent,
      ...
      if (isLocked) {
        throw new AccountLockedError(
          'Tenant account has been locked due to consecutive failed login attempts.',
        );
      }
      throw new AuthenticationError('Invalid email or password');
    }
    ...
    const orgCheck = await this.validateOrganizationTenantStatus(
      credential.tenantId,
    );
    if (!orgCheck.isValid) {
      await authRepository.recordLoginAttempt({
      ...
      throw new UnauthorizedError(
        `Salon organization is ${orgCheck.status || 'inactive'}. Please contact support.`,
      );
    }
    ...
    const safeProfile: CachedTenantProfile = {
    ...
    await identityReadStore.getTenantAuthProfile(
      credential.tenantId,
      async () => safeProfile,
    );
  ...
  public async getMe(
    ...
    const profile = await identityReadStore.getTenantAuthProfile(
      tenantId,
      async () => {
    ...
    const effectiveAccess = await identityReadStore.getTenantEffectiveAccess(
      tenantId,
      async () => ({
        tenantId,
  ...
  public async forgotPassword(email: string): Promise<{ message: string }> {
    ...
    const credential =
      await tenantCredentialRepository.findByNormalizedEmail(normalized);
    ...
    if (credential) {
      ...
      try {
        ...
        logger.info(
          ...
          'Generated tenant password reset token',
        );
    ...
    return {
      message:
        'If a salon account with that email exists, password reset instructions have been sent.',
    };
  }
  ...
  public async resetPassword(
    ...
    return {
      message:
        'Salon password has been reset successfully. Please login with your new password.',
    };
  }
}
