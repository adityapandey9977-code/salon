...
import { ConsumptionWastageTab } from './ConsumptionWastageTab';
import { InventoryOverviewTab } from './InventoryOverviewTab';
import { InventoryReportsTab } from './InventoryReportsTab';
import { ProcurementTab } from './ProcurementTab';
import { ProductsConsumablesTab } from './ProductsConsumablesTab';
...
export interface InventoryMasterPageProps {
  roleTitle?: string;
...
export function InventoryMasterPage({
  ...
  parentSection = 'Commercial',
  defaultBranch = 'All',
  lockBranch = false,
  initialTab = 'overview',
}: InventoryMasterPageProps = {}) {
  const [searchParams, setSearchParams] = useSearchParams();
  const navigate = useNavigate();
  const activeTabParam = (searchParams.get('tab') as InventoryTabType) || initialTab;
  ...
  const tabs: TabItem[] = [
    {
      ...
      icon: Package,
      description: lockBranch
        ? `Real-time stock valuation, inventory health, and shelf-life distribution for ${defaultBranch}`
        : 'Multi-branch inventory health, KPIs, and shelf-life distribution',
    },
    {
      id: 'products',
    ...
    },
    {
      id: 'stock',
      label: 'Stock',
      count: lockBranch ? '2.8K Units' : '14.8K Units',
      icon: Package,
    ...
    },
    {
      id: 'transfers',
      label: 'Transfers',
      count: lockBranch ? '2 Active' : '6 Active',
      icon: ArrowRightLeft,
      description: 'Stock transfers, dispatch orders, and chain-of-custody tracking',
    },
    {
      id: 'consumption',
      label: 'Consumption & Wastage',
      count: lockBranch ? '₹8.2K' : '₹34.5K',
      icon: Scissors,
    ...
    },
    {
      id: 'stocktake',
      label: 'Stocktake',
      count: lockBranch ? '1 Audit' : '5 Audits',
      icon: ClipboardCheck,
  ...
  const renderActiveTabContent = () => {
    switch (activeTabParam) {
      ...
      case 'stock':
        return <StockTab defaultBranch={defaultBranch} lockBranch={lockBranch} />;
      case 'procurement':
        return <ProcurementTab />;
      case 'transfers':
        return <TransfersTab />;
      case 'consumption':
        return <ConsumptionWastageTab defaultBranch={defaultBranch} lockBranch={lockBranch} />;
      case 'stocktake':
        return <StocktakeTab defaultBranch={defaultBranch} lockBranch={lockBranch} />;
      case 'reports':
          ...
          <div className="flex-1 max-w-2xl">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <span className="px-2.5 py-0.5 rounded-full bg-white/15 backdrop-blur-md text-[10px] font-extrabold tracking-wider uppercase text-purple-100 flex items-center gap-1 border border-white/10">
                ...
                {lockBranch
                  ? `Branch Manager · ${defaultBranch}`
                  : '  Salon SaaS · Head Office Control'}
              </span>
              <span className="text-white/40 text-xs">•</span>
              <span className="text-purple-200 text-xs font-medium">
                {lockBranch ? 'Outlet Stock & Consumables' : 'Multi-Branch Inventory Visibility'}
              </span>
            </div>
            <h1 className="text-2xl md:text-3xl font-serif font-bold tracking-tight text-white">
              Inventory, Retail &amp; Stock Master Desk
            </h1>
            <p className="text-purple-100/80 text-xs md:text-sm mt-1 font-light leading-relaxed">
              {lockBranch
                ? `Real-time outlet stock valuation for ${defaultBranch}, service recipe BOMs, purchase requisition governance, inter-branch transfers, consumption variance, and physical stocktake reconciliation.`
                : 'Real-time multi-branch stock valuation, service recipe BOMs, purchase order governance, inter-branch transfers, consumption variance, and physical stocktake reconciliation.'}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5 shrink-0 flex-wrap sm:flex-nowrap justify-start lg:justify-end">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3 border border-white/15 min-w-[120px]">
              ...
              <strong className="text-lg font-serif font-bold text-amber-300 block mt-0.5">
                {lockBranch ? '8 SKUs' : '42 SKUs'}
              </strong>
              <span className="text-[10px] text-purple-200 font-semibold block mt-0.5">
                {lockBranch ? '1 Critical Out of Stock' : '8 Critical Out of Stock'}
              </span>
            </div>

            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3 border border-white/15 min-w-[120px]">
              <span className="text-[10px] uppercase tracking-wider text-purple-200 block font-bold">
                {lockBranch ? 'Pending In-Flight' : 'Pending Procurement'}
              </span>
              <strong className="text-lg font-serif font-bold text-white block mt-0.5">
                {lockBranch ? '2 In-Transit' : '12 Orders'}
              </strong>
              <span className="text-[10px] text-purple-200 font-semibold block mt-0.5">
                {lockBranch ? '₹85,000 In-Flight' : '₹4.85L In-Flight Value'}
              </span>
            </div>
          </div>
        </div>
