...
export class RabbitMQEventBus {
  ...
  public async publish<T extends Record<string, unknown>>(
    ...
    const key =
      routingKey || `${event.aggregateType.toLowerCase()}.${event.eventType.toLowerCase()}`;
    ...
    try {
      ...
      if (!this.channel) {
        console.warn(
          `[RabbitMQEventBus] Channel not available, buffered event: ${event.eventType}`,
        );
        return false;
      }
  ...
  public async subscribe<T extends Record<string, unknown>>(
    routingKey: string,
